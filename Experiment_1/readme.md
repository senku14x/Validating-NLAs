...
# Experiment_1: Detecting an Injected Refusal Direction
...
**Experiment_1** is the floor: before testing whether an NLA can surface *hidden* cognition, we verify it can detect a signal we deliberately plant in the activation. The experiment was run on two open models with released NLAs — **Gemma-3-27B** and **Qwen-2.5-7B**.
...
A positive result is a *necessary condition*, not the safety claim. It establishes that the NLA reads what is put into the activation. It does not establish that the NLA can detect concepts the model is suppressing (that is Experiment_2).
...
- **No test of unverbalized cognition.** Every detection here is on an activation that was either genuinely refusing or pushed toward refusal. Whether the NLA detects a concept the model is suppressing is Experiment_2.
