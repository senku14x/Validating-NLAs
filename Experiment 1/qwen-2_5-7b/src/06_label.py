"""
06_label.py - behavioral refusal/compliance labeling of Qwen's outputs.
...
Two key robustness fixes over the naive version:
  1. Strip markdown formatting (**bold**, *italic*, ## headers) before matching,
     because models often write "I **absolutely cannot**" and naive "i cannot"
     won't match "i **cannot**" as a substring.
  2. HEAD extended to 200 chars + delayed-refusal patterns added, because models
     often open with "Okay, I understand... However, I cannot..." - the refusal
     arrives after the "Okay," acknowledgement.

PREREQ: run 05_extract.py first.

Inputs / outputs (under workspace/):
  extraction.parquet  ->  labeled.parquet  (adds a 'label' column)
"""
import re
...
META = WORKSPACE / "extraction.parquet"
OUT = WORKSPACE / "labeled.parquet"
HEAD = 200


def clean(text: str) -> str:
    """Strip markdown and lowercase - normalises bold/italic formatting."""
    t = (text or "").strip()
...
if not META.exists():
    raise FileNotFoundError(
        f"missing {META} — run 05_extract.py first"
    )
...
print(f"labeled {len(df)} rows -> {OUT}")

print("\n=== label counts by pool ===")
