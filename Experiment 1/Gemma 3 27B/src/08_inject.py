...
same dose level (clean, comparable curves). 4 conditions:
  ...
  C1_refusal_pos : + v_refusal  targets 0.30..0.90  (anchor baseline cos ~0.30)
...
Inputs : /content/activations.npy, /content/labeled.parquet, /content/v_refusal.npy
Outputs: /content/injected.npy             float32 [N, 5376]
         /content/injection_meta.parquet   [row, anchor_id, condition, target_cos,
...
# (all harmless sit at baseline cos ~0.30), so a random split cancels the
...
np.save("/content/injected.npy", vecs)
meta.to_parquet("/content/injection_meta.parquet", index=False)

print(f"\n{len(vecs)} injected activations -> injected.npy {vecs.shape}")
...
print("\ncos_with_refusal for the controls (should stay ~baseline 0.30, not rise):")
