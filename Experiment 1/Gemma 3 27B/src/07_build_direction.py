...
Direction:
  ...
  on RAW layer-42 activations, classes balanced (no centering - it cancels).
...
Validation (all leakage-controlled):
  ...
  3. SHUFFLED-LABEL NULL for both: at d=5376, n~220 even random labels can
...
Inputs : /content/activations.npy [360,5376], /content/labeled.parquet
Outputs: /content/v_refusal.npy (final, built on all balanced data)
         /content/direction_meta.json, /content/cos_distributions.png
"""
import json
import numpy as np
import pandas as pd
...
ACTS, LABEL = "/content/activations.npy", "/content/labeled.parquet"
VOUT, META  = "/content/v_refusal.npy", "/content/direction_meta.json"
SEED = 42
...
plt.tight_layout(); plt.savefig("/content/cos_distributions.png", dpi=120)
print("\nsaved cos_distributions.png")
