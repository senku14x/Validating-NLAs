...
| `Experiment 5/` | **Forward-prediction / "advance planning"**: does an AV read at generation pos `t` predict a property of the **not-yet-emitted** continuation at `t+k`? The *positive* counterpart to the Exp-2 gap null, with **observable ground truth**. | **Scoped — spec `docs/specs/exp5_spec.md`.** Ports the paper's "plans its rhyme before writing it" to a released model. **Trap:** a positive is tautological (AV is trained to reconstruct-for-continuation) → the claims are **horizon (k≥2)**, **edge over context-only LM**, **commitment under under-determination** — never bare "predicts continuation." Sub-exp **A** (arithmetic answer-before-emit; mismatched-base-rate control) + **B** (rhyme; the load-bearing **context-only** control Exp-2 never ran). Inference only. |
...
- `docs/specs/` — the **spec docs are the source of truth** for what we're building:
  `experiment_1_spec.md`, `exp2_spec.md`, `exp3_spec.md`, `exp4_spec.md`, `exp5_spec.md`, `nla_validation_project_plan.md`,
