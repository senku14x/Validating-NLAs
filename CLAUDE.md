...
| `Experiment 3/` | **Cross-model** feature transfer: ridge/Procrustes map Qwen-L20→Gemma-L41; co-firing AUROC + causal steering. Standalone. | **§9 layer correction applied** (62 layers, L41≈66% = near-depth-matched). Pilot N=50 self-mined: coarse map great (recon 0.998, SVCCA 0.97) but **feature-identity transfer sparse (2/50 eligible) & domain-confounded** (survivors are broad domain/format features). **Next: same-domain hard negatives** (load-bearing control), full standardized-Procrustes 05/06, NLA-vs-cofiring comparison — **not** scale-to-N=500. Work on branch `claude/exp3-cross-model-transfer`. |
