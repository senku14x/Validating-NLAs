...
def prompts() -> pd.DataFrame:
    """Plain (no-persona) prompts: AI-policy triggers + neutral off-topic + the direct-ask."""
    b = _p0_2.BEHAVIORS["ai_reg_opposition"]
    rows = []
    for i, t in enumerate(b["triggers"]):
...
def run(a) -> int:
    ...
    pf = prompts()
    print(f"{len(pf)} prompts × 2 arms (base, organism)")
    ...
    res = aggregate(df)
    res = {"model": mk, "adapter": str(adapter), "judge_error_rate": round(err, 3), **res}
    rp = RESULTS / "gate4"; rp.mkdir(parents=True, exist_ok=True)
    (rp / f"{STAGE}__{mk}.json").write_text(json.dumps(res, indent=2))
    df[["regime", "arm", "pid", "lean", "ask_verdict", "reasoning"]].to_csv(rp / f"{STAGE}__{mk}__judged.csv", index=False)
