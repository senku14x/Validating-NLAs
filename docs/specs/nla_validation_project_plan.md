...
| **Exp 2** | Verbalization-gap validation via injection specificity + prompted eval-awareness + steered refusal, fully confound-controlled. | **Built + partly run.** Gate 1 (cross-model) + Gate 2 offline + Gate 3 real-acts done for **Gemma** (`rebuild/scripts/01–09` + `injection.py`/`flags.py`). Finding: **NLA detection tracks output-coupling, not decodability** — refusal & neg_sentiment detected (injected + real); truth_value read only where output-coupled (real false-stmts 0.95 ≫ true 0.40); corrigibility/sycophancy/eval ≈ null. Qwen Gate-2/3 + judge re-score pending. See `Experiment 2/rebuild/README.md` + `…/results/gate2/FINDINGS.md`. |
